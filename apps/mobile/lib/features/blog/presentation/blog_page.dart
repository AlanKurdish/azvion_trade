...
class _BlogPageState extends State<BlogPage> {
  ...
  List<dynamic> _posts = [];
  bool _buying = false;
  // Non-null when the last load failed (network / server error). Kept separate
  // from "subscribed but zero posts" so a transient failure shows a retry
  // instead of masquerading as the paywall or an empty "No posts" screen.
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      // Load balance for the subscribe-confirmation modal — non-fatal if it fails.
      try {
      ...
      if (_subscribed) {
        try {
          ...
          _posts = data is List ? data : <dynamic>[];
        } on DioException catch (e) {
          if (e.response?.statusCode == 403) {
            // Backend reports no active subscription after all → show paywall.
            _subscribed = false;
            _posts = [];
          } else {
            // Real failure (timeout / 5xx / offline). Don't hide it as "no posts".
            rethrow;
          }
        }
      } else {
      ...
      // The status call (or a genuine posts failure) blew up — surface a retry
      // rather than silently dropping the user onto the paywall.
      _loadFailed = true;
    } finally {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _row(
                '${t.tr('subscribeFor')} (${t.tr('monthly')})',
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(
            '${t.tr('subscriptionExpires')} $remaining',
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            clipBehavior: Clip.antiAlias,
            // Fixed card height keeps every compact card uniform and lets the
            // thumbnail (BoxFit.cover) crop tall/portrait posters instead of
            // stretching the row to the image's natural aspect ratio. The fixed
            // height also bounds CrossAxisAlignment.stretch, which would
            // otherwise force an infinite height inside the ListView.
            child: SizedBox(
              height: 104,
              child: Row(
