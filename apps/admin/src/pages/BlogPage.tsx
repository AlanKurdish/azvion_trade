...
export default function BlogPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!form.title.trim() || !form.content.trim()) {
      return setError(t('blog.titleContentRequired'));
    }
    setSaving(true);
    try {
      const payload: any = {
      ...
      setError(err.response?.data?.message || t('blog.saveFailed'));
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (p: Post) => {
    if (!confirm(t('blog.confirmDelete', { title: p.title }))) return;
    try {
      await api.delete(`/blog/${p.id}`);
      load();
    } catch (err: any) {
      alert(err.response?.data?.message || t('blog.deleteFailed'));
    }
  };
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <FileText className="text-[#D4AF37]" size={26} /> {t('blog.title')}
          </h2>
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <span className={`px-2 py-0.5 rounded text-[10px] font-semibold shrink-0 ${p.isPublished ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'}`}>
                  {p.isPublished ? t('blog.live') : t('blog.draft')}
                </span>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-[#1e293b] p-6 rounded-xl border border-[#334155] w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold">{editing ? t('blog.editPost') : t('blog.createPost')}</h3>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div>
                <label className="block text-xs text-gray-400 mb-1">{t('blog.postTitle')}</label>
              ...
              <div>
                <label className="block text-xs text-gray-400 mb-1">{t('blog.content')}</label>
                <RichTextEditor
                  value={form.content}
                  onChange={(html) => setForm({ ...form, content: html })}
                  placeholder={t('blog.writeHere')}
                  minHeight={280}
              ...
              <label className="flex items-center gap-2 text-sm text-gray-300">
                ...
                {t('blog.publishedVisible')}
              </label>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowForm(false)} className="flex-1 py-2 border border-[#334155] rounded-lg text-gray-300">{t('blog.cancel')}</button>
                <button type="submit" disabled={saving} className="flex-1 py-2 bg-[#D4AF37] text-black rounded-lg font-semibold disabled:opacity-50">
                  {saving ? t('blog.saving') : editing ? t('blog.save') : t('blog.publish')}
                </button>
              </div>
            </form>
          </div>
